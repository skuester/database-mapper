...
class Table {
	...
	query(opts = {}) {
		let list = opts.query_list || new QueryList({
			DB: this.DB,
		})
		...
		list.push(new Query({
			...
			// define factory
			TABLES[name] = function DbTable(opts = {}) {
				this.path = opts.path || this.name
				this._query = opts.query || new Query({root_table: this.name}, DB.knex)
				this.prefix = opts.prefix || this._query.get_prefixer(this.path)
			}

			TABLES[name].prototype = new DbTablePrototype(table_config, DB)
	}
